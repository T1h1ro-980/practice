...
def show_multiple_augmentations(original_img, augmented_imgs, titles, fontsize = 10):
    ...
    axes[0].set_title("Оригинал", fontsize = fontsize)
    axes[0].axis('off')
    ...
    for i, (aug_img, title) in enumerate(zip(augmented_imgs, titles)):
        ...
        axes[i + 1].set_title(title, fontsize = fontsize)
        axes[i + 1].axis('off')
